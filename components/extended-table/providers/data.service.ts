...
export class DataService<T> {
    ...
    getRowData(rowData): ETRowData<T> {
        return {
    ...
    getData() {
        if (!this.groupingService.isDataGrouped) return this.sortingService.getSortedData(this.data);
        // construct the groups
        let groups = this.data?.reduce((groups, row) => this.customReducer(groups, row), {});
        // sort group data
        for (const key in groups) {
            const groupItem = groups[key].shift();
            groups[key] = [groupItem, ...this.sortingService.sortData(groups[key])];
        }
